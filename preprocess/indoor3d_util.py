...
g_class2color = {'DontCare':	[0,255,0],
                 'cyclist':	[0,0,255],
                 'tricycle':	[0,255,255],
                 'sm allMot':        [255,255,0],
                 'bigMot':      [255,0,255],
...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
    ...
    for idx in range(len(xbeg_list)): 
       ...
       cond = xcond & ycond
       if np.sum(cond) < 30: # discard block if there are less than 100 pts. kx 100 30
           continue
       
       block_data = data[cond, :]
       block_label = label[cond]
       # print(block_label.shape)
       #筛选太严格 会导致选不出来1个block 保证1frame至少有1block
       if(drop<1 and len(block_data_list)>0):#drop class0 background
           rand_0_1 = random.random()
           distributions,_ = np.histogram(block_label, list(range(NUM_CLASS + 1)))
           distributions = distributions/np.sum(distributions)
           if(distributions[0]==1):#全是背景    0.001
               # print("total background",end='')
               if (rand_0_1 > drop):  # drop=0.1 只有当drop<0.1时才保留 选择了原来的0.1倍样本
                   continue
               else:
                   pass
                   # print("  reserve it!")
           elif (distributions[3]>0.5):#class 3 比较多
               print("class 3: ",distributions[3],end='')
               if (rand_0_1 > 50*drop):  # 0.05
                   continue
               else:
                   print("  reserve it!")
           elif (distributions[4]>0.5):#class 4 比较多
               print("class 4: ",distributions[4],end='')
               if (rand_0_1 > 500*drop):  # 0.5
                   continue
               else:
                   print("  reserve it!")
           elif (distributions[7]>0.5):#class 7 比较多
               print("class 4: ",distributions[7],end='')
               if (rand_0_1 > 500*drop):  # 0.5
                   continue
               else:
                   print("  reserve it!")
