...
os.system('cp train_xyzi_final_iter_all_iou_loss.py %s' % (LOG_DIR))  # bkp of train procedure
...
def create_graph(iter):
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            batch_label_num = tf.placeholder(tf.int8, shape=())
            # loss = MODEL.get_loss(pred, labels_pl, smpws_pl) #soft entropy
            loss = MODEL.get_iou_loss_with_num(pred, labels_pl, num=batch_label_num)
            loss = MODEL.get_iou_loss(pred,labels_pl)
            ...
            # IOU = loss
            # get iou
            IOU,mean_IOU = MODEL.get_iou(pred,tf.one_hot(labels_pl,NUM_CLASSES),NUM_CLASSES)
            # print(IOU)
            for cls in range(1,NUM_CLASSES-1):
                print(cls)
                tf.summary.scalar('IoU/'+ str(cls), IOU[cls])
            tf.summary.scalar('IoU/IoU_mean', mean_IOU)
        ...
        ops = {'pointclouds_pl': pointclouds_pl,
               ...
               'pred': pred,
               'batch_label_num': batch_label_num,
               'loss': loss,
               'IOU' : IOU,
               'train_op': train_op,
               'merged': merged,
...
def train_one_epoch(sess, ops, train_writer,train_data, train_label):
    ...
    for batch_idx in tqdm(range(num_batches)):
        ...
        #number of labels in this batch
        batch_label_num = np.unique(current_label[start_idx:end_idx]).shape[0]
        ...
        feed_dict = {ops['pointclouds_pl']: current_data[start_idx:end_idx, :, 0:4],#xyz i
                     ...
                     ops['batch_label_num']: batch_label_num,
                     ops['is_training_pl']: is_training, }
        summary, step, _, loss_val, pred_val,_ = sess.run([ops['merged'], ops['step'],
                                                         ops['train_op'], ops['loss'], ops['pred'],ops['IOU']],
                                                        feed_dict=feed_dict)
...
def eval_one_epoch(sess, ops, test_writer, test_data, test_label):
    ...
    for batch_idx in range(num_batches):
        ...
        feed_dict = {ops['pointclouds_pl']: current_data[start_idx:end_idx, :, 0:4],#xyz i
                     ...
                     ops['batch_label_num']: batch_label_num,
                     ops['is_training_pl']: is_training}
        summary, step, loss_val, pred_val,_ = sess.run([ops['merged'], ops['step'],
                                                      ops['loss'], ops['pred'],ops['IOU']], feed_dict=feed_dict)
