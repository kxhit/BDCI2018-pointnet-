...
# DATA_PATH = os.path.join('/media/DATA/dataset/Stanford3dDataset_v1.2/Stanford3dDataset_v1.2_Aligned_Version') #kx
# g_classes = [x.rstrip() for x in open(os.path.join(BASE_DIR, 'meta/class_names.txt'))]
g_classes = [x.rstrip() for x in open(os.path.join(BASE_DIR, 'meta/alibaba_class_names.txt'))]
g_class2label = {cls: i for i,cls in enumerate(g_classes)}
g_class2color = {'DontCare':	[0,255,0], #绿色
                 'cyclist':	[0,0,255], #蓝色
                 'tricycle':	[0,255,255], #青色
                 'sm allMot':        [255,255,0], #黄色
                 'bigMot':      [255,0,255], #紫色
                 'pedestrian':      [255,0,0], #红色
                 'crowds':        [255, 160, 122], #橙色
                 'unknown':       [255,255,255]} #白色
g_easy_view_labels = [1,6,5,4,3,2] #kx object
...
def sample_data(data, num_sample):
    ...
    elif (N > num_sample):#N>目标值 降采样
        sample = np.random.choice(N, num_sample,False) # replace = False 不允许重复采样   True 可以重复 会出现相同的元素
        return data[sample, ...], sample
    # elif (N > num_sample):# 按顺序采样 多的再采
    #     # print(data[0:num_sample, ...].shape)
    #     return data[0:num_sample, ...], list(range(num_sample))
    else:#N<目标值 过采样
...
def sample_data_label(data, label, num_sample):
    ...
    new_id = sample_indices
    return new_data, new_label, new_id
    
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
                random_sample=False, sample_num=None, sample_aug=1, drop=1.0):
    ...
    #assumes the data is shifted (min point is origin)
    ...
    for idx in range(len(xbeg_list)): 
       ...
       if np.sum(cond) <= 0: # discard block if there are less than 100 pts. kx 100 30
           continue
       ...
       # print(np.where(cond == True))
       # print(np.array(np.where(cond==True)))
       block_id = np.squeeze(np.array(np.where(cond==True)),0) #原始的index

       # print(block_id.shape)
       # print(block_id)
       total_points_num += block_id.shape[0]
       # print(block_label.shape)
       # if(drop<1):
       #     rand_0_1 = random.random()
       #     tmp = block_label.copy()
       #     tmp.sort()
       #     if(tmp[-1]==0): #label升序 全是背景
       #          print("total background")
       #          if (rand_0_1>drop):#drop=0.1 只有当drop<0.1时才保留 选择了原来的0.1倍样本
       #              print("discard it!")
       #              continue

       # 判断 data size 和 num_point
       loop_num = math.ceil(block_data.shape[0]/num_point) # 要重复采样的次数
       new_id = range(block_id.shape[0]) #block_data 的 id new_id, sampled_id, remain_id
       remain_id = new_id #初始化
       total_sampled_id = []
       # print(remain_id)
       # print(block_data.shape)
       block_points_num = 0
       for loop in range(loop_num):
           # orderly subsample data to batch  Reserve every point!
           block_data_sampled, block_label_sampled, sampled_id = \
               sample_data_label(block_data[remain_id,:], block_label[remain_id], num_point)
           remain_id = np.array(remain_id)
           sampled_id = remain_id[sampled_id]
           # print("sampled id")
           # print(sampled_id)
           # print(set(sampled_id))
           #要传block_data的剩余index
           block_id_sampled = block_id[sampled_id]
           block_points_num += len(set(block_id[sampled_id]))
           block_data_list.append(np.expand_dims(block_data_sampled, 0))  # 继续append sample剩下的data 直到data没了
           block_label_list.append(np.expand_dims(block_label_sampled, 0))
           block_id_list.append(np.expand_dims(block_id_sampled, 0))

           remain_id = [] #重新初始化
           total_sampled_id += list(set(sampled_id))
           total_sampled_id = list(set(total_sampled_id))
           # print(total_sampled_id)
           if(loop_num==1):#只需要跑一次 省点时间
               continue
           # print("new id")
           # print(new_id)
           # print("total_sampled id")
           # print(total_sampled_id)

           # for item in new_id:
           #     if item not in total_sampled_id:
           #         remain_id.append(item)
           remain_id = list(set(new_id).difference(total_sampled_id))
           # print("remain_id")
           # print(remain_id)
           # print(len(remain_id))
           # print("loop: %d",loop)
       ...
       # print(len(new_id))
       # print(len(total_sampled_id))
       # assert len(new_id) == len(total_sampled_id)
       # print(block_points_num)
       # print(block_id.shape[0])
       # assert block_id.shape[0]==block_points_num
    ...
    # print("total_points_num")
    # print(total_points_num)
    # print("id list len")
    # print(len(np.unique(block_id_list)))
    # print(len(block_id_list))
    return np.concatenate(block_data_list, 0), \
           np.concatenate(block_label_list, 0), \
           np.concatenate(block_id_list, 0)


def room2blocks_plus(data_label, num_point, block_size, stride,
                     random_sample, sample_num, sample_aug, drop=1.0):
    ...
    return room2blocks(data, label, num_point, block_size, stride,
                       random_sample, sample_num, sample_aug,drop=drop)
...
def room2blocks_plus_normalized(data_label, num_point, block_size, stride,
    ...
    data_batch, label_batch, id_batch = room2blocks(data, label, num_point, block_size, stride,
                                          random_sample, sample_num, sample_aug,drop=drop)
    # print(len(np.unique(id_batch)))
    # print(data.shape[0])
    # print(id_batch.shape[0])
    assert len(np.unique(id_batch)) == data.shape[0]
    new_data_batch = np.zeros((data_batch.shape[0], num_point, 9))
    for b in range(data_batch.shape[0]):#50ms
        new_data_batch[b, :, 6] = data_batch[b, :, 0]/max_room_x
        new_data_batch[b, :, 7] = data_batch[b, :, 1]/max_room_y
        new_data_batch[b, :, 8] = data_batch[b, :, 2]/max_room_z
        minx = min(data_batch[b, :, 0])#每个block里做了normalize!!! 而不是全局
        miny = min(data_batch[b, :, 1])
        data_batch[b, :, 0] -= (minx+block_size/2)
        data_batch[b, :, 1] -= (miny+block_size/2)
    new_data_batch[:, :, 0:6] = data_batch
    return new_data_batch, label_batch, id_batch
...
def room2blocks_wrapper_normalized(data_label_filename, num_point, block_size=1.0, stride=1.0,
    ...
    if data_label_filename[-3:] == 'txt':
        ...
        print('Unknown file type! exiting.')
        exit()
    print("file %s",data_label_filename)
    print("points size: %d",data_label.shape[0])
    return room2blocks_plus_normalized(data_label, num_point, block_size, stride,
                                       random_sample, sample_num, sample_aug,drop=drop)

# def room2blocks_wrapper_normalized_outdoor(data_label_filename, num_point, block_size=1.0, stride=1.0,
#                                    random_sample=False, sample_num=None, sample_aug=1):
#     if data_label_filename[-3:] == 'txt':
#         data_label = np.loadtxt(data_label_filename)
#     elif data_label_filename[-3:] == 'npy':
